...
class ImageProcessor:
    ...
    def _resolve_grid_layout(
        ...
        best_layout: Optional[Tuple[int, int]] = None
        best_x_edges: Optional[np.ndarray] = None
        best_y_edges: Optional[np.ndarray] = None
    ...
    def _edges_from_all_gaps(self, gaps: List[Tuple[int, int, float]], size: int) -> Optional[np.ndarray]:
        if not gaps:
            return None
    ...
    def _build_edges_from_centers(self, size: int, centers: List[int]) -> Optional[np.ndarray]:
        if not centers:
            return None
    ...
    def _compute_content_bounds(self, alpha: np.ndarray, threshold: int = 8) -> Optional[Tuple[int, int, int, int]]:
